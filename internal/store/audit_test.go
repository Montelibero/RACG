...
func TestUpdateRequestStatus(t *testing.T) {
	...
	if err := s.UpdateRequestStatus(ctx, "req1", "APPROVED"); err != nil {
		t.Fatalf("UpdateRequestStatus: %v", err)
	}
	got, err := s.GetRequest(ctx, "req1")
	if err != nil {
		t.Fatalf("GetRequest: %v", err)
	}
...
func TestListSessionHistoryItems(t *testing.T) {
	ctx := context.Background()
	...
	items, err := s.ListSessionHistoryItems(ctx, "sess1", 50)
	if err != nil {
		t.Fatalf("ListSessionHistoryItems: %v", err)
	}
	if len(items) != 1 {
		t.Fatalf("items=%d, want 1", len(items))
	}
	if items[0].RequestID != "req1" {
		t.Fatalf("request_id=%q, want req1", items[0].RequestID)
	}
	if items[0].Decision == nil || *items[0].Decision != "ALLOW_ONCE" {
		t.Fatalf("decision=%v, want ALLOW_ONCE", items[0].Decision)
	}
	if items[0].ExecutionStatus == nil || *items[0].ExecutionStatus != "SUCCEEDED" {
		t.Fatalf("exec_status=%v, want SUCCEEDED", items[0].ExecutionStatus)
	}
}
