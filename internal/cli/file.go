...
func (c *FileCmd) runRead(args []string) int {
	...
	return c.submitFileRequest(*host, *token, *noWait, *pollInterval, *waitTimeout, "fs.read", payload)
}
...
func (c *FileCmd) runPatch(args []string) int {
	...
	return c.submitFileRequest(*host, *token, *noWait, *pollInterval, *waitTimeout, "fs.patch_unified", payload)
}

func (c *FileCmd) submitFileRequest(host, token string, noWait bool, pollInterval, waitTimeout time.Duration, opType string, payload map[string]any) int {
