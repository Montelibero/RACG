...
func (e *Engine) Match(sessionID string, op Op) (Match, bool) {
	...
	if op.Type == "cmd.run" {
		...
		if analysis.Shell {
			if r, source, ok := matchExactCmdArgvRule(sess, always, analysis.OriginalArgv); ok {
				return Match{RuleID: r.ID, Source: source}, true
			}
		}
...
func (e *Engine) Explain(sessionID string, op Op) Explanation {
	...
	if op.Type == "cmd.run" {
		...
		if analysis.Shell {
			if r, source, ok := matchExactCmdArgvRule(sess, always, analysis.OriginalArgv); ok {
				return Explanation{Allowed: true, Segments: []SegmentDecision{{
					Argv:       append([]string(nil), analysis.OriginalArgv...),
					SourceText: strings.Join(analysis.OriginalArgv, " "),
					Source:     source,
					RuleID:     r.ID,
					Allowed:    true,
				}}}
...
func matchExactCmdArgvRule(sess, always []Rule, argv []string) (Rule, string, bool) {
	for _, candidate := range []struct {
		source string
		rules  []Rule
	}{{source: "session", rules: sess}, {source: "always", rules: always}} {
		for _, rule := range candidate.rules {
			if rule.OpType != "cmd.run" || rule.Cmd == nil || rule.Cmd.TailAny || len(rule.Cmd.ArgvPrefix) != len(argv) {
				continue
			}
			exact := true
			for i := range argv {
				if strings.Contains(rule.Cmd.ArgvPrefix[i], "*") || rule.Cmd.ArgvPrefix[i] != argv[i] {
					exact = false
					break
				}
			}
			if exact {
				return rule, candidate.source, true
			}
		}
	}
	return Rule{}, "", false
